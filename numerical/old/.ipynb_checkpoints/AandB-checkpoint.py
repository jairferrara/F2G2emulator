...
CONFIG = {
    ...
    "output_curves"   : "AandB_curves.txt",
}
...
def main(CONFIG, curves=False):
    ...
    if curves:
        C_num = 8
        C_dens = 100
        z_arr, k1_arr, k2_arr, x12_arr, om_arr, log10fR0_arr = generate_samples(C_num)
        print(z_arr.shape, k1_arr.shape)
        z_arr = np.tile(np.linspace(0, 3, C_dens), k1_arr.shape[0])
        k1_arr, k2_arr, x12_arr, om_arr, log10fR0_arr = np.repeat(k1_arr, C_dens), np.repeat(k2_arr, C_dens), np.repeat(x12_arr, C_dens), np.repeat(om_arr, C_dens), np.repeat(log10fR0_arr, C_dens)
        print(z_arr.shape, k1_arr.shape)
    else:
        t0 = time.time()
    ...
    if curves:
        write_results(CONFIG["output_curves"], params, batch_results)
    else:
